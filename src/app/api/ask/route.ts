...
export async function POST(req: NextRequest) {
  try {
    ...
    const mcpApiKey = process.env.GLOBAL_CHURCH_API_KEY || process.env.GC_API_KEY;
    if (!mcpUrl) {
    ...
    if (!mcpApiKey) {
      console.error('MCP API key not configured. Set GLOBAL_CHURCH_API_KEY or GC_API_KEY');
      return NextResponse.json(
