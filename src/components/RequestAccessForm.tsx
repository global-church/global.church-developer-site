...
type RequestAccessFormProps = {
  /** Pre-fill values for logged-in users */
  defaults?: {
    fullName?: string;
    email?: string;
    company?: string;
...
export function RequestAccessForm({ defaults, successMessage }: RequestAccessFormProps) {
  ...
  const invalid =
    req(data.fullName) || req(data.email) || req(data.company) || req(data.useCase) || !data.termsAccepted;
      ...
      <div className="rounded-xl border border-green-200 bg-green-50 p-6 text-green-900 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-green-800 mt-1">
            {successMessage ??
              "Thanks for reaching out. We'll review your details and contact you shortly with next steps."}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-1 gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <Input
            ...
            aria-invalid={req(data.fullName)}
        ...
        <div>
          ...
          <Input
            ...
            value={data.company}
            onChange={onFieldChange}
            aria-invalid={req(data.company)}
            placeholder="Your Organization"
            required
          />
        </div>
        <div>
          <label htmlFor="roleTitle" className="mb-2 block text-sm font-medium text-gray-700">
            Role / Title
          </label>
          <Input
            id="roleTitle"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <Input
            ...
            onChange={onFieldChange}
            placeholder="App or initiative name"
          />
        </div>
        <div>
      ...
      <div>
        <label htmlFor="useCase" className="mb-2 block text-sm font-medium text-gray-700">
          Use case / project description<span className="text-red-500">*</span>
        </label>
        <Textarea
          id="useCase"
          name="useCase"
          value={data.useCase}
          onChange={onFieldChange}
          aria-invalid={req(data.useCase)}
          placeholder="Describe what you're building, who it serves, and how you plan to use the API."
          required
        />
      </div>
        ...
        <div>
          <fieldset>
            ...
            <div className="grid grid-cols-1 gap-2 text-sm text-gray-700">
              {['Church search', 'Church details', 'Geospatial (nearby/within)', 'Bulk export', 'Other'].map(
                (label) => (
                  <label key={label} className="flex items-center gap-3">
                    <input
                      type="checkbox"
                      className="size-4 rounded border-gray-300"
                      checked={data.endpoints.includes(label)}
                      onChange={() => toggleEndpoint(label)}
                    />
                    <span>{label}</span>
                  </label>
                ),
        ...
        <div className="grid grid-cols-1 gap-6">
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setData((d) => ({ ...d, expectedVolume: v }))}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Average requests/day" />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            <label htmlFor="regions" className="mb-2 block text-sm font-medium text-gray-700">
              Regions of interest
            </label>
            <Input
              id="regions"
              ...
              value={data.regions}
              onChange={onFieldChange}
              placeholder="e.g., North America, Europe, Sub-Saharan Africa"
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          <label htmlFor="heardFrom" className="mb-2 block text-sm font-medium text-gray-700">
          ...
          <Input
            ...
            onChange={onFieldChange}
            placeholder="Referral, FaithTech, search, etc."
          />
        </div>
      </div>
      ...
      <div>
        ...
        <Textarea
          ...
          value={data.notes}
          onChange={onFieldChange}
          placeholder="Additional context, constraints, or questions."
        />
      </div>
      ...
      <div className="flex items-start gap-3">
        ...
        <label htmlFor="termsAccepted" className="text-sm text-gray-700">
          I agree to responsible use of the API and understand that access may be adjusted to protect platform
          stability and data quality. <span className="text-red-500">*</span>
        </label>
      </div>
